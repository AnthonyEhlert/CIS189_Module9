...
The purpose of this program is to import the my_definitions.py module and call the functions contained in the module

The input is a dictionary of items and a set of items that will be passed into two of the functions from the imported
module

The output is prints to the console that are called from each function of the imported module
"""
...
if __name__ == "__main__":
    ...
    # call the four functions contained in the imported module
    md.greeting()
    md.message()
    md.print_dict(my_dict)
    md.print_set(my_set)
    ...
    def test_greeting(self):
        ...
        try:
            ...
            try:
                ...
                print(f"{bcolors.WARNING}You appear to be missing a print statement within your greeting function." + bcolors.ENDC)
    ...
    def test_message(self):
        ...
        try:
            ...
            try:
                ...
                print(f"{bcolors.WARNING}You appear to be missing a print statement within your message function." + bcolors.ENDC)
            #Check for empty print statement
            if len(output) < 3:
                print(f"{bcolors.WARNING}Your print statement is empty." + bcolors.ENDC)
            else:
    ...
    def test_print_dict(self):
        ...
        try:
            ...
            try:
                ...
                print(f"{bcolors.WARNING}You appear to be missing a print statement within your print_dict function; or one was never called." + bcolors.ENDC)
            if len(output) < 3:
                print(f"{bcolors.WARNING}Your print statement is empty." + bcolors.ENDC)
            else:
                if (output == "(Test 1, 1)\n(Test 2, Joe)\n(Test 3, 26.33)\n(Test 4, ['a', 'small', 'list'])\n"):
                    ...
                    print(f"{bcolors.WARNING}With a dictionary of a_dict = {{'Test 1':1, 'Test 2':'Joe', 'Test 3':26.33, 'Test 4': ['a','small','list']}}" + bcolors.ENDC)
                    print(f"{bcolors.WARNING}Expected output: " + bcolors.ENDC)
                    print(f"{bcolors.OKBLUE}(Test 1, 1)\n(Test 2, Joe)\n(Test 3, 26.33)\n(Test 4, ['a', 'small', 'list']" + bcolors.ENDC)
            ...
            if "positional arguments" in str(te):
                print(f"{bcolors.WARNING}Potentially missing a parameter/argument for 'print_dict()'; should contain only 1 parameter." + bcolors.ENDC)
                print(f"{bcolors.WARNING}Original Error: \n\t" + str(te) + bcolors.ENDC)
            elif "can only concatenate str" in str(te):
                print(f"{bcolors.WARNING}Dictionary print statement didn't handle values other than a dictionary of strings." + bcolors.ENDC)
                print(f"{bcolors.WARNING}Suggestion: Add some type casting, such as casting an int to a string." + bcolors.ENDC)
                print(f"{bcolors.WARNING}Original Error: \n\t" + str(te) + bcolors.ENDC)
            else:
    ...
    def test_print_set(self):
        ...
        try:
            ...
            try:
                ...
                print(f"{bcolors.WARNING}You appear to be missing a print statement within your print_set function; or one was never called." + bcolors.ENDC)
            if len(output) < 3:
                print(f"{bcolors.WARNING}Your print statement is empty." + bcolors.ENDC)
            else:
                ...
                if (4 == output.count('\n') and "Apple" in output and "Banana" in output and "6\n" in output and "55.67\n" in output):
                    ...
                    split_text = output.split("\n")
                    print(f"{bcolors.WARNING}Some piece of your print statement didn't work" + bcolors.ENDC)
                    print(f"{bcolors.WARNING}Note that sets are unordered to actual output might be in a different order than expected...that is fine." + bcolors.ENDC)
            ...
            if "positional arguments" in str(te):
                print(f"{bcolors.WARNING}Potentially missing a parameter/argument for 'print_set()'; should contain only 1 parameter." + bcolors.ENDC)
                print(f"{bcolors.WARNING}Original Error: \n\t" + str(te) + bcolors.ENDC)
            elif "can only concatenate str" in str(te):
                print(f"{bcolors.WARNING}Dictionary print statement didn't handle values other than a dictionary of strings." + bcolors.ENDC)
                print(f"{bcolors.WARNING}Suggestion: Add some type casting, such as casting an int to a string." + bcolors.ENDC)
